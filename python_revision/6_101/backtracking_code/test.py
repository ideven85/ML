...
import graph_version as mixtape
...
def test_mixtape_examples():
    ...
    _test_valid(songs, 11, mixtape.mixtape_graph(dict(songs), 11))
    assert mixtape.mixtape_graph(dict(songs), 1000) is None
    _test_valid(songs, 21, mixtape.mixtape_graph(dict(songs), 21))
...
def test_mixtape_from_file(n):
    ...
    result = mixtape.mixtape_graph(dict(songs), target)
    if valid:
