    ...
    for _ in range(10_000):
        for a1, a2, v in tests:
            ...
            if a1 != a2:
                ...
                assert res is False and isinstance(
                    res, bool
                ), f"expected False for {a1} and {a2} acting together, got {res}"
...
def test_movie_path_03():
    ...
    k = random.randint(4, 7)
    assert (
        lab.actors_connecting_films(lab.transform_data(make_bacon_tree(N, k)), 0, 1)
        is None
    )
    assert (
        lab.actors_connecting_films(lab.transform_data(make_bacon_tree(N, k)), 0, 1)
